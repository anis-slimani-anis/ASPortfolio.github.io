...
(function initDotGrid() {
  ...
  function sync() {
    const isLight = document.documentElement.getAttribute('data-theme') === 'light';
    const onProjectPage = !!document.querySelector('nav.work-header');
    if (!isLight || onProjectPage) { unmount(); return; }
    mount();
  }
  ...
  function tick() {
    ...
    curX += (tgtX - curX) * 0.06;
    curY += (tgtY - curY) * 0.06;
    el.style.backgroundPosition = `${curX.toFixed(2)}px ${curY.toFixed(2)}px`;
  ...
  window.addEventListener('mousemove', e => {
    tgtX = (e.clientX / window.innerWidth  - 0.5) * 18;
